...
def ComputeSSD(TODOPatch, TODOMask, textureIm, patchL):
	...
	for r in range(ssd_rows):
		for c in range(ssd_cols):
			...
			total_sum = 0 # keep track of our ssd sum

			for x in range(2*patchL+1):
				# iterate over each x coordinate in TODOPatch
				for y in range(2*patchL+1):
					# iterate over each y coordinate in TODOPatch
					
					if TODOMask[x-patchL][y-patchL] == 0:
						# ignore mask values of 1 to skip
						pVal = TODOPatch[x-patchL][y-patchL]*1.0
						# take the patch value offset by where we are in the original image
						# also coerece it to be a float
						imgVal = textureIm[r+x-patchL][c+y-patchL]*1.0
						# take the image value offset also coerece it to be a float

						val = pVal-imgVal
						total_sum += val**2

			SSD[r][c] = total_sum[0] + total_sum[1] + total_sum[2]
	return SSD
...
def CopyPatch(imHole,TODOMask,textureIm,iPatchCenter,jPatchCenter,iMatchCenter,jMatchCenter,patchL):
	...
	for i in range(patchSize):
		for j in range(patchSize):
			...
			patchX = i + iPatchCenter - patchL
			patchY = j + jPatchCenter - patchL

			imgX = i + iMatchCenter - patchL
			imgY = j + jMatchCenter - patchL

			if TODOMask[i][j] == 1:
				imHole[patchX][patchY] = textureIm[imgX][imgY] 
	return imHole
